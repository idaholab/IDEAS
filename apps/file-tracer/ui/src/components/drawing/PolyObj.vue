...
export default {
  ...
  computed: {
    id() {
      return this.polyline_obj.index;
    },
    points() {
      let points_string = "";
      let x = 0;
      let y = 0;
      for (var j=0;j<this.polyline_obj.points.length;j++) {
        x = this.get_x(this.polyline_obj.points[j][0]);
        y = this.get_y(this.polyline_obj.points[j][1]);
        points_string += `${x},${y} `;
      }
      return points_string;
    },
    stroke_width() {
      return this.get_stroke(this.polyline_obj.linewt);
    },
  },
  methods: {
    sendID(idx) {
