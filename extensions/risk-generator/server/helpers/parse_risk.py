...
def parse_risks(data):
    ...
    date_risk_identified_timestamp = data['attrs'].get('481')
    if (date_risk_identified_timestamp is not None):
        date_risk_identified = datetime.fromtimestamp(int(date_risk_identified_timestamp / 1000))
    else:
        date_risk_identified = None

    date_risk_closes_timestamp = data['attrs'].get('482')
    if (date_risk_closes_timestamp is not None):
        date_risk_closes = datetime.fromtimestamp(int(date_risk_closes_timestamp / 1000))
    else:
        date_risk_closes = None
    ...
    risk = {
        ...
        'impacted_wbs_elements': data['attrs'].get('483'),
        ...
        'risk_sub_category': data['attrs'].get('480'),
