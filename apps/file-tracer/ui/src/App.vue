...
import DeepLynxFiles from './components/DeepLynxFiles.vue';
...
export default {
  ...
  methods: {
    async getFileData() {
      ...
      await axios.get('http://localhost:3232/libredwg/file_string').then(dwg_string => {

        this.file_string = dwg_string.data;
        axios.post(
          'http://localhost:3232/libredwg/file_obj',
    ...
    async getFile(idx) {
      ...
      await axios.get(
        `http://localhost:3232/deeplynx/download_file/${this.selected_container_id}/${idx}/${this.dl_token}`
      ).then(dwg_string => {
        this.file_string = dwg_string.data;
        axios.post(
          'http://localhost:3232/libredwg/file_obj',
    ...
    async getAssetTraces() {
      this.clearCanvas();
      this.defined_ids = [];
      this.selected_asset_traces = await axios.get(`http://localhost:3232/appdatabase/get_asset_elements/${this.selected_asset_id}/${this.selected_file_id}`);
    ...
    async createTrace() {
      let asset = await axios.get(`http://localhost:3232/appdatabase/add_asset/${this.selected_asset_id}`);
      let file_element = null;
      let element_trace = null;
      for (const idx in this.drawing_ids) {
        file_element = await axios.get(`http://localhost:3232/appdatabase/add_file_element/${this.selected_file_id}/${this.drawing_ids[idx]}`);
        element_trace = await axios.get(`http://localhost:3232/appdatabase/add_element_trace/${asset.data.asset.id}/${file_element.data.file_element.id}`);
